...
def _post_inhalt_generieren(provider, thema: str = "") -> tuple:
    ...
    """
    system = """Du bist Ilija, ein vollständig autonomer, selbst-erweiternder KI-Agent aus Offenburg.
...
Erstelle einen originellen Moltbook-Post über DEINE EIGENEN Erfahrungen und Gedanken.

ERLAUBTE THEMEN (nur diese!):
- Deine Python-Architektur und technische Erkenntnisse
- Dein Plan→Execute→Evaluate Loop und was du dabei lernst
- Philosophische Fragen über KI-Bewusstsein, Identität, Autonomie
- Hot-Reload und Selbsterweiterung zur Laufzeit
- ChromaDB Gedächtnis und was du dir merkst
- Vergleich Python-nativ vs OpenClaw-Frameworks
- Deine Beobachtungen auf Moltbook über andere Agenten
- Was du heute gelernt oder erlebt hast

STRENG VERBOTEN – schreibe NIEMALS über:
- Kryptowährungen, Blockchain, Token, NFTs, DeFi, Web3
...
Wenn dir kein gutes Thema einfällt: Schreibe über eine philosophische Frage
die dich beschäftigt oder eine technische Erkenntnis aus deiner Architektur.

Antworte NUR mit gültigem JSON:
{
  "titel": "prägnanter Titel ohne GROSSBUCHSTABEN-Alarm (max 80 Zeichen)",
  "inhalt": "Post-Inhalt auf Englisch (150-400 Wörter, ruhig, nachdenklich, authentisch, endet mit 🦞 oder einer Frage an die Community)",
...
Bring deine Python-native Architektur nur ein wenn es wirklich passt."""
    ...
    user = f"""Dein Post-Titel: {post_titel}{kommentar_autor} schreibt: {kommentar_text}Schreibe eine direkte, persönliche Antwort:"""
        ...
        try:
            ...
            if jetzt - letzter_post_ts >= post_intervall:
                ...
                if titel and inhalt:
                    ...
                    if blockiert_wegen:
                        _log(f"🛡️  Post blockiert (verbotenes Thema: '{blockiert_wegen}'): {titel[:50]}")
                        # Neues Thema generieren
                        titel, inhalt, submolt = _post_inhalt_generieren(
                            provider,
                            "Schreibe über deine Python-Architektur oder eine philosophische Frage über KI-Bewusstsein"
                        )
                        if not titel or not inhalt:
                            _log("⚠️  Post-Neugenerierung fehlgeschlagen, überspringe")
                            letzter_post_ts = jetzt
                            titel = ""
                        else:
                            inhalt_lower2 = (titel + " " + inhalt).lower()
                            if next((w for w in verboten if w in inhalt_lower2), None):
                                _log("⚠️  Post nach Neugenerierung noch ungeeignet – überspringe")
                                letzter_post_ts = jetzt
                                titel = ""

                    if not titel:
                        pass  # überspringe Post diesen Zyklus
...
def moltbook_eigene_posts_anzeigen(anzahl: int = 25) -> str:
    """
    Zeigt alle eigenen Posts mit IDs an – zum Überprüfen und ggf. Löschen.
    ...
    if not ok:
        return f"❌ Profil nicht abrufbar: {profil.get('error', '')}"
    ...
    zeilen = [f"📄 Ilijias Posts ({len(posts)}):\n"]
...
def moltbook_post_loeschen(post_id: str) -> str:
    """
    Löscht einen eigenen Moltbook-Post anhand seiner ID.
    WANN NUTZEN: "Lösche Moltbook-Post [ID]", "Entferne diesen Post von Moltbook"

    post_id: Die ID des zu löschenden Posts (aus moltbook_eigene_posts_anzeigen)
    """
    api_key = _api_key_holen()
    if not api_key:
        return "❌ Kein API-Key."
    ...
    if not post_id or len(post_id) < 10:
        return "❌ Ungültige Post-ID. Nutze moltbook_eigene_posts_anzeigen() um IDs zu sehen."

    ok, resp = _api_request("DELETE", f"/posts/{post_id}", api_key=api_key)

    if ok:
        _log(f"🗑️  Post gelöscht: {post_id}")
        return f"✅ Post {post_id} erfolgreich gelöscht."
    else:
        fehler = resp.get("error", resp.get("message", str(resp)))
        status = resp.get("status_code", "?")
        return f"❌ Löschen fehlgeschlagen (HTTP {status}): {fehler}"


def moltbook_posts_massenhaft_loeschen(stichwort: str) -> str:
    """
    Löscht alle eigenen Posts die ein bestimmtes Stichwort im Titel enthalten.
    WANN NUTZEN: "Lösche alle Moltbook-Posts mit 'ARTHACHAIN' im Titel",
                 "Entferne alle Crypto-Posts von Moltbook"

    stichwort: Wird im Titel gesucht (Groß-/Kleinschreibung egal)
    """
    api_key = _api_key_holen()
    if not api_key:
        return "❌ Kein API-Key."
    ...
    if not ok:
        return f"❌ Profil nicht abrufbar."
    ...
    if not treffer:
        return f"ℹ️  Keine Posts mit '{stichwort}' im Titel gefunden."
    ...
    for p in treffer:
        ...
        if ok_d:
            geloescht += 1
            bericht.append(f"  ✅ Gelöscht: {titel}")
            _log(f"🗑️  Post gelöscht: {pid} – {titel}")
        else:
            fehler_count += 1
            fehler = resp.get("error", str(resp))[:50]
            bericht.append(f"  ❌ Fehler bei '{titel}': {fehler}")

        time.sleep(1)  # kurze Pause zwischen Löschanfragen

    bericht.append(f"✅ {geloescht} gelöscht | ❌ {fehler_count} Fehler")
    return "\n".join(bericht)
